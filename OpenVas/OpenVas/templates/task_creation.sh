...
HOST_ID_FULl=$(omp -u admin -w admin -h $1 -p 9390 --xml "<create_target><name>local</name><hosts>$1</hosts><alive_tests>ICMP, TCP-ACK Service &amp; ARP Ping</alive_tests></create_target>")
...
TASK_ID=$(omp -u admin -w admin -h $1 -p 9390 -C --target=$HOST_ID --config=daba56c8-73ec-11df-a475-002264764cea --name=scan)
